...
void useit(int client_fd, char *buffer) //获取返回的响应报文
{
	int i;
	char uri[100];
	char method[5];
...
void getdynamic(int fd1)
{
	...
	else
	{
		...
		sprintf(buff2, "%sContent-Length: %ld\r\n", buff2, strlen(last));
...
int err(int f, char *ret)
{
	...
	case 3:
		f = 403;
		strcpy(ret, "fobbiden permission");
		break;
	case 4:
		f = 501;
		strcpy(ret, "not implement");
		break;
	}
	return f;
}
